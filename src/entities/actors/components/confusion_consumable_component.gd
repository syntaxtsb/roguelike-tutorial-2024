...
func activate(action: ItemAction) -> bool:
	...
	if not map_data.get_tile(action.target_position).is_in_view:
		...
		return false
	if not target:
		MessageLog.send_message("You must select and enemy to target.", GameColors.IMPOSSIBLE)
		return false
	if target == consumer:
		MessageLog.send_message("You cannot confuse yourself!", GameColors.IMPOSSIBLE)
		return false
	
	MessageLog.send_message("The eyes of the %s look vacant, as it starts to stumble around!"
			% target.get_entity_name(), GameColors.STATUS_EFFECT_APPLIED)
